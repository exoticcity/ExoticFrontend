                ...
                {userObj?.CustomerPostingGroup === "LOCAL" || userObj?.CustomerPostingGroup === "PARTICULIER" ? (
                  <>
                    <Typography variant="h6" sx={{ fontSize: '12px', fontFamily: 'Monteserrat' }}>
                      {t('TotalExVat')}: {totalPrice.total_amount_excluding_vat}
                    </Typography>
                    <Typography variant="h6" sx={{ fontSize: '12px', fontFamily: 'Monteserrat' }}>
                      {t('Vat Amount')}:  {totalPrice.vat_amount}
                    </Typography>
                    <Typography variant="h6" sx={{ fontSize: '12px', fontFamily: 'Monteserrat' }}>
                      {t('TotalInVat')}: {totalPrice.total_amount_including_vat}
                    </Typography>
                  </>
                ) : (
                  <>
                    <Typography variant="h6" sx={{ fontSize: '12px', fontFamily: 'Monteserrat' }}>
                      {t('Total Amount')}: € {isPricesLoading ? 'Loading...' : (isNaN(totalAmoutExcVat) ? '0.00' : totalAmoutExcVat.toFixed(2))}
                    </Typography>
                  </>
                )}
              </>
